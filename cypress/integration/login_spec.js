...
describe('Community Signup View', () => {
  ...
  it('should render', () => {
    ...
    cy.get(
      '[href*="/auth/github?r=http://staging.weirdstreet.com/spectrum"]'
    ).should('be.visible');
    cy.get('[href*="github.com/withspectrum/code-of-conduct"]').should(
      'be.visible'
    );

    cy.get('[href*="/login?r=http://staging.weirdstreet.com/spectrum"]').should(
      'be.visible'
    );
  });
});
...
describe('Redirect paths', () => {
  it('should preserve community redirect paths', () => {
    ...
    cy.get('[href*="/login?r=http://staging.weirdstreet.com/spectrum"]')
      .should('be.visible')
      .click();

    cy.get(
      '[href*="/auth/twitter?r=http://staging.weirdstreet.com/spectrum"]'
    ).should('be.visible');
    cy.get(
      '[href*="/auth/facebook?r=http://staging.weirdstreet.com/spectrum"]'
    ).should('be.visible');
    cy.get(
      '[href*="/auth/google?r=http://staging.weirdstreet.com/spectrum"]'
    ).should('be.visible');
    cy.get(
      '[href*="/auth/github?r=http://staging.weirdstreet.com/spectrum"]'
    ).should('be.visible');
    cy.get(
      '[href*="/new/user?r=http://staging.weirdstreet.com/spectrum"]'
    ).should('be.visible');
  });

  it('should preserve thread redirect paths', () => {
    const path =
      'http://staging.weirdstreet.com/spectrum/general/yet-another-thread~thread-3';
    cy.visit(path);
