...
describe('Toasts and url query paramaters', () => {
  ...
  it('should show toast', () => {
    const url = new URL('http://staging.weirdstreet.com/me/settings');
    ...
    cy.url().should(
      'eq',
      'http://staging.weirdstreet.com/users/mxstbr/settings'
    );
  });
  ...
  it('should preserve existing query parameters', () => {
    const url = new URL(
      'http://staging.weirdstreet.com/spectrum/general/another-thread~thread-2?m=MTQ4MzIyNTIwMDAwMg=='
    );
    url.searchParams.append('toastType', 'success');
    url.searchParams.append('toastMessage', toastMessage);
    cy.visit(url.toString());
