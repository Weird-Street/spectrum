...
slackRouter.get('/', (req: any, res: any) => {
  ...
  return generateOAuthToken(code, returnURI)
    ...
    .then(slug => {
      return IS_PROD
        ? res.redirect(`https://staging.weirdstreet.com/${slug}/settings`)
        : res.redirect(`http://staging.weirdstreet.com/${slug}/settings`);
    });
});
...
slackRouter.get('/onboarding', (req: any, res: any) => {
  ...
  return generateOAuthToken(code, returnURI)
    ...
    .then(id => {
      return IS_PROD
        ? res.redirect(
            `https://staging.weirdstreet.com/new/community?s=2&id=${id}`
          )
        : res.redirect(
            `http://staging.weirdstreet.com/new/community?s=2&id=${id}`
          );
    });
});
